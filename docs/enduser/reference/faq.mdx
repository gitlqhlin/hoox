...
No. Hoox is designed for **algorithmic traders**, not blockchain developers. On-chain execution is handled by `web3-wallet-worker` (Uniswap V3 + Jupiter Swap V2). Send the same webhook as CEX with `exchange: "uniswap-ethereum"` (or `uniswap-arbitrum` / `jupiter-solana`) and a `BASE/QUOTE` symbol. See [DEX spot swaps](/docs/enduser/guides/dex-spot-swaps).
...
Test fills are stored as `TEST_EXECUTED` with `*-testnet-*` position ids so they do not mix with live exposure. DEX venues reject `test: true`. See **[Test Trading](/docs/enduser/guides/test-trading)**.

### Q: How do I swap on Uniswap or Jupiter?

Use `exchange: "uniswap-ethereum"` | `"uniswap-arbitrum"` | `"jupiter-solana"` and `symbol: "ETH/USDC"` (slash required). `hoox setup` writes the wallet mesh key, a Solana seed if missing, and public RPCs. You still set `JUPITER_API_KEY` for Solana. Guide: **[DEX spot swaps](/docs/enduser/guides/dex-spot-swaps)**.
...
To change the interval, update `triggers.crons` and redeploy. See [agent-worker Cron setup](/docs/devops/workers/agent-worker#cron-schedule-11440-minutes).
...
<Note>
  Open an issue on
  [GitHub](https://github.com/hoox-sh/hoox/issues) or join our
