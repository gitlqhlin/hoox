...
export function searchConcepts(concepts: ConceptDoc[], terms: string[]): Hit[] {
  ...
  for (const concept of concepts) {
    ...
    const tags = Array.isArray(concept.frontmatter.tags)
      ...
      : "";
    const haystack =
      `${concept.id} ${title} ${description} ${resource} ${tags}`.toLowerCase();
    let score = 0;
    for (const needle of needles) {
      if (
        concept.id.toLowerCase() === needle ||
        resource.toLowerCase() === needle
      )
        score += 8;
      else if (
...
export function renderContext(
  ...
  const root =
    conceptById(concepts, start) ??
    searchConcepts(concepts, start.split(/[\s/]+/)).map((hit) =>
      concepts.find((concept) => concept.id === hit.id)
    )[0];
