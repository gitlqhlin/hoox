...
Operator guide: [Test Trading](/docs/enduser/guides/test-trading).

---

## DEX venues (P1)

Spot swaps route through `WEB3_WALLET_SERVICE` (`web3-wallet-worker`). P1 is spot-only; perps venues land in P2–P4.

| Name | Chain | Kind | Toggle | Quantity | Test flag |
| ---- | ----- | ---- | ------ | -------- | --------- |
| `uniswap-ethereum` | ethereum | spot | `exchange:uniswap-ethereum:enabled` | `openLong` spends QUOTE (ExactIn) to buy BASE; `openShort` sells BASE | Rejected (`TEST_TRADING_UNSUPPORTED`) — mainnet-only |
| `uniswap-arbitrum` | arbitrum | spot | `exchange:uniswap-arbitrum:enabled` | same | Rejected — mainnet-only |
| `jupiter-solana` | solana | spot | `exchange:jupiter-solana:enabled` | same | Rejected — mainnet-only |

Uniswap venues send `dexVersion: "v3"` (ETH/USDC liquidity is V3; Arbitrum's V2 router in the wallet worker is SushiSwap). Native `ETH`/`SOL` map to the wrapped mint on contract paths. Credential source is `wallet-binding` (mesh key), not CEX API secrets.

Operator guide: [DEX spot swaps](/docs/enduser/guides/dex-spot-swaps). `hoox setup` writes `WALLET_EXECUTE_KEY_BINDING` automatically.
...
- **[hoox Gateway Profile](/docs/devops/workers/hoox)** — Review WAF rules and Durable Object idempotency locks.
- **[D1 Database Operations](/docs/enduser/guides/database-ops)** — Manage Drizzle schemas, migrations, and query operations.
