...
/** Worker checkouts are git submodules. Their source is not part of this bundle. */
const SKIP_PREFIXES = ["docs/images"];
...
export function classifyPath(path: string): InputFile["kind"] | null {
  ...
  if (
    CODE_EXT.has(ext) &&
    CODE_ROOTS.some((root) => path === root || path.startsWith(`${root}/`))
  )
    return "code";
  if (CODE_ROOTS.some((root) => path === root || path.startsWith(`${root}/`)))
    return "other";
  return null;
}
...
export function collectInputs(root: string, fromIndex: boolean): InputFile[] {
  ...
  const unstaged = fromIndex
    ? new Set(nulSplit(git(root, ["diff", "-z", "--name-only"])))
    : new Set<string>();
  ...
  for (const path of paths.sort()) {
    ...
    if (kind !== "other") {
      try {
        ...
        const size =
          fromIndex && unstaged.has(path)
            ? MAX_PARSE_BYTES
            : statSync(abs).size;
        if (size <= MAX_PARSE_BYTES)
          text = readText(root, path, fromIndex, unstaged);
      } catch {
        text = "";
      }
      if (text.includes("\0")) {
    ...
    files.push({
      path,
      text,
      kind: text === "" && kind !== "other" ? "other" : kind,
    });
  }
  return files;
}
...
export function readBundleFromIndex(
  root: string,
  bundleRel = "okf"
): Map<string, string> {
