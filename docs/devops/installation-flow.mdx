...
**Setup gates (0.11.x):** onboard will **not** run setup if `init` was cancelled or never wrote `wrangler.jsonc`. Setup also aborts when worker trees are still missing after the submodule clone attempt, and requires Cloudflare® auth (`wrangler login` or `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`).

To start the system bootstrap, run the one-shot wizard from the monorepo root
(the CLI remembers that path so later `hx` calls work from any directory):
...
| `full`     | All workers (AI agent, DeFi wallet, email, pyne-worker / Pine Script™, …) + integrations |
...
- **[DevOps Setup & Operations Manual](/docs/devops/setup-and-operations)** — Dive into complete operations, variable matrices, and troubleshooting.
- **[Terminal UI Cockpit](/docs/devops/tui)** — Run, hot-reload, and monitor your local workers via TUI.
- **[CLI features](/docs/devops/cli-features)** — Monorepo remember path, secrets modes, Linear Rail banner.
