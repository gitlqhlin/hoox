...
| **Pine Script™**       | TradingView®'s domain-specific language for indicators and strategies. Hoox docs use `//@version=6`. Edge evaluate also runs via **pyne-worker** / `hoox pyne`.       |
| **Leverage**           |
...
| **DEX venue**          | Spot exchange name routed through `web3-wallet-worker`: `uniswap-ethereum`, `uniswap-arbitrum`, `jupiter-solana`. Pairs use `BASE/QUOTE`. Mainnet-only.               |
| **ExactIn**            | Swap mode that spends a fixed input amount (`quantity`) and accepts a slippage-bounded output. DEX `LONG` spends quote to buy base.                                  |
| **SwapRouter02**       | Uniswap V3 periphery router used on Ethereum and Arbitrum (`dexVersion: "v3"`). Native ETH unwraps via `unwrapWETH9`.                                                |
| **Jupiter Swap V2**    | Solana aggregator API (`GET /order` + `POST /execute` at `api.jup.ag/swap/v2`). Requires `JUPITER_API_KEY`.                                                          |
...
| **Remembered monorepo**           | Path stored in `~/.hoox/config/monorepo.json` after the CLI discovers a local checkout, so `hx` works from any working directory. |
...
| **Wrangler**                      | Cloudflare®'s official CLI for Workers. Hoox embeds it; prefer `hoox` / `hx` for operator workflows.                           |
| **Linear Rail banner**            | Default CLI banner style (`◆ H · O · O · X`) with tagline/version; compact TTY animation.                                      |
