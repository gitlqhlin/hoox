...
- Hoox is a free, open-source, edge-native algorithmic trading framework. It ingests trade signals from TradingView webhooks, email parsers, and Telegram commands, then routes signed orders to centralized exchanges or DEX venues (Uniswap V3 / Jupiter Swap V2).
...
- Supported CEX venues are **Binance**, **Bybit**, and **MEXC**. DEX spot venues are **`uniswap-ethereum`**, **`uniswap-arbitrum`**, and **`jupiter-solana`** (ExactIn via `web3-wallet-worker`). DEX pairs use `BASE/QUOTE` and reject `test: true`.
