...
\textsc{HOOX} consists of ten core production Workers (Table~\ref{tab:workers}) that form the trading mesh, plus optional Pine tooling isolates (\texttt{pine-worker}, \texttt{pyne-worker}) that evaluate strategies and can forward events into \texttt{trade-worker} over Service Bindings with mesh internal auth. Counting the OpenNext dashboard as a compute surface yields eleven operator-facing surfaces in the monorepo.
...
\begin{table}[t]
  ...
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}llp{5.5cm}l@{}}
    ...
    \texttt{agent-worker} & Risk manager & Trailing stops, take-profit, kill switch & Cron (default \texttt{*/15}) \\
...
\begin{enumerate}
  ...
  \item \textbf{Persistence and notification:} Results are written to D1 via \texttt{d1-worker}, logged to R2, and forwarded to \texttt{telegram-worker} and \texttt{analytics-worker} via non-blocking \texttt{waitUntil} calls. Because D1 writes are intentionally non-blocking for latency, the ledger can lag fills; \texttt{trade-worker} therefore exposes \texttt{POST /api/positions/reconcile} so exchange open interest is treated as source of truth.
  \item \textbf{Risk management:} On a configurable cron (default every fifteen minutes), \texttt{agent-worker} runs a deterministic loop---trailing stops, take-profit scale-out, drawdown kill switch, housekeeping health probes, and position reconciliation against live exchanges. Event-driven risk via the WebSocket connection manager remains future work.
\end{enumerate}
