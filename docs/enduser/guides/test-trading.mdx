...
| `uniswap-ethereum` | No | Rejected (`TEST_TRADING_UNSUPPORTED`) |
| `uniswap-arbitrum` | No | Rejected (`TEST_TRADING_UNSUPPORTED`) |
| `jupiter-solana` | No | Rejected (`TEST_TRADING_UNSUPPORTED`) |

MEXC has no public REST sandbox for automated trading (UI demo only). DEX venues are mainnet-only (Uniswap V3 / Jupiter Swap V2). Requests with `test: true` on those names fail with a 400 `TEST_TRADING_UNSUPPORTED`. See [DEX spot swaps](/docs/enduser/guides/dex-spot-swaps).
...
- [Signals & Trade Spec](/docs/enduser/concepts/signals-and-trades) — full webhook field table
- [Secrets & Network Security](/docs/enduser/guides/secrets-security) — secret injection
- [API endpoints](/docs/enduser/reference/api-endpoints) — request examples
