...
- **`SOLANA_PRIVATE_KEY`**: Base58-encoded 32-byte seed or 64-byte Phantom secret. `wrangler secret put`; never logged.
- **`JUPITER_API_KEY`**: `x-api-key` for `https://api.jup.ag/swap/v2`. `wrangler secret put`.
- **`RPC_URL_SOLANA`**: Solana JSON-RPC (or KV `wallet:rpc:solana`).
