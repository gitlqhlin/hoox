...
description: "How to configure TradingView® alerts, write copy-paste Pine Script™ v6 indicators, and link webhook signals to your edge gateway."
---
...
2. The `hoox` gateway validates the signature and routes the order to the `exchange` in the payload (Bybit in this example; DEX venues use `BASE/QUOTE` — [DEX spot swaps](/docs/enduser/guides/dex-spot-swaps)).
...
| :------------------------------------ | :-------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------- |
| **Alert fires but no trade executes** | Mismatched API keys or WAF block.       | Run `hoox logs worker hoox` to stream traffic. Check if the client IP was dropped by Cloudflare® WAF.                                         |
...
- **[Setting Up Telegram Alerts](/docs/enduser/tutorials/telegram-bot)** — Integrate Telegram notifications to get fills pushed directly to your phone.
- **[API Endpoint Reference](/docs/enduser/reference/api-endpoints)** — Review full request schemas and status codes.
