...
\paragraph{Decision.} Split production logic into ten core Workers (Table~\ref{tab:workers}) plus optional Pine tooling isolates, communicating exclusively via Service Bindings except for outbound exchange HTTPS. A canonical binding registry in \texttt{hoox-shared} drives CLI deployment ordering and documentation sync (Listing~\ref{lst:registry-hoox}).
...
\paragraph{Context.} The repository contains the core worker mesh, optional \texttt{pyne-worker} tooling, three shared packages (CLI, TUI, \texttt{hoox-shared}), tests, and paper tooling---all sharing TypeScript types and middleware where applicable.
