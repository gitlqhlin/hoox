...
graph TB
    ...
    subgraph Compute["⚡ Private Internal Edge Compute"]
        ...
        PYNE["🐍 pyne-worker (Pine Script™ edge eval — tooling)"]:::worker
    end
    ...
    subgraph Storage["💾 Persistent Edge Storage"]
        ...
        DO{{"🔒 Idempotency + RateLimiter DOs"}}:::compute
    ...
    PYNE -.->|R2 OHLCV / scripts| R2
    PYNE -->|TRADE_SERVICE + X-Internal-Auth-Key| TW
```
...
| **`report-worker`**    | Browser Rendering PDF (`BROWSER`) | Cron `08:00` and `18:00` UTC |    No (Isolated)     |  **Yes** (Rendering APIs)  |     Print Status      |
| **`email-worker`**     | Mailgun / Email Routing / JSON | No inbox poll |    No (Isolated)     |             No             |   Parse Statistics    |
| **`web3-wallet-worker`** | DeFi Swap Engine |      No      |    No (Isolated)     | **Yes** (RPC proxied) |     Tx Sign Logs      |
| **`analytics-worker`** |  Observability   |      No      |    No (Isolated)     |             No             |    Metrics Dataset    |
| **`pyne-worker`**      | Pine edge evaluate | Cron `* * * * *` | **Tooling** (`API_KEY` / `X-API-Key`) | **Yes** (evaluate path) | Evaluate / forward logs |
| **`dashboard`**        | Next.js ops UI   |      No      | **Yes** (public)     |             No             |    UI / binding calls |
...
- **[Worker Communication Specifications](/docs/devops/architecture/communication)** — Deep dive into service bindings, zero-TCP routing, and V8 engines.
- **[Data Flow Maps](/docs/devops/architecture/data-flow)** — Step-by-step sequence charts of trade executions and cron risk evaluations.
- **[Codebase Dependency Graph](/docs/devops/architecture/overview#-codebase-dependency-graph)** — Generate a live architecture map of all exports, calls, and bindings.
