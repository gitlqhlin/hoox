...
Test fills are stored as `TEST_EXECUTED` with `*-testnet-*` position ids so they do not mix with live exposure. See **[Test Trading](/docs/enduser/guides/test-trading)**.
