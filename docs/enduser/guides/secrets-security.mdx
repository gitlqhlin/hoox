...
# 4. DEX: Jupiter portal key (Solana). Mesh WALLET_EXECUTE + Solana seed come from hoox setup.
hoox secrets set web3-wallet-worker JUPITER_API_KEY

# 5. After key rotation: push mesh/system secrets only (INTERNAL_KEY_BINDING, WALLET_EXECUTE_*, WEBHOOK_*, …)
hoox secrets sync --system
# Alias: hoox secrets sync --required

# 6. Push all declared secrets from .dev.vars
