...
```bash
# Profile 1: workers — 10 services, no dashboard UI
#   hoox, trade-worker, telegram-worker, d1-worker, web3-wallet-worker,
#   agent-worker, email-worker, report-worker, analytics-worker, pyne-worker
...
# Profile 3: full — 11 services (the 10 workers + dashboard)
...
`hoox dev start` assigns the ports below. agent-worker, email-worker, report-worker, and analytics-worker are not pinned and take the next free port from `8800` upward, in enable order. Docker Compose publishes only gateway `8787` and dashboard `8794`. Inside the compose network, report-worker, analytics-worker, and pyne-worker also pass `--port 8797`, `8798`, and `8799`; those ports are not published to the host.

| Microservice           | `hoox dev` port | Local Binding URL       | Purpose                   |
| :--------------------- | :-------------: | :---------------------- | :------------------------ |
| **`hoox`**             |     `8787`      | `http://localhost:8787` | Ingress Gateway           |
| **`trade-worker`**     |     `8788`      | `http://localhost:8788` | Order Execution Engine    |
| **`d1-worker`**        |     `8789`      | `http://localhost:8789` | Relational SQLite Manager |
| **`telegram-worker`**  |     `8790`      | `http://localhost:8790` | Notifications Hub         |
| **`web3-wallet-worker`** |   `8792`      | `http://localhost:8792` | On-Chain DeFi Node        |
| **`dashboard`**        |     `8794`      | `http://localhost:8794` | Next.js Dashboard         |
| **`pyne-worker`**      |     `8799`      | `http://localhost:8799` | Pine Script™ edge evaluate |
...
- [**Testing Standards**](/docs/devops/development/testing) — Run unit and integration tests using Bun's native test runner.
- [**Debugging Runbook**](/docs/devops/development/debugging) — Debug local and remote isolates, tail logs, and trace queries.
