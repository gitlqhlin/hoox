...
5. **`INTERNAL_AUTH_KEY`**, **`HOOX_API_KEY`**, **`LOAD_TEST_BASE_URL`**: Required for nightly k6 load tests (see [Security Overview](/docs/devops/security/overview#environment-setup)).
...
- **[Production Deployment Manual](/docs/devops/deployment/production)** — Audit DNS zones and custom domain routing options.
- **[System Observability & Monitoring](/docs/devops/deployment/monitoring)** — Stream console logs and check analytics datasets.
