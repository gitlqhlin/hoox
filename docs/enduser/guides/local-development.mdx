...
Run from the monorepo (or any directory after the CLI has remembered that path —
see [Installation](/docs/enduser/getting-started/installation#run-from-any-directory-monorepo-remember)).
...
```bash
# Profile 1: Workers only (10 services — all background workers, no UI)
...
`hoox dev start` pins the ports below. agent-worker, email-worker, report-worker, and analytics-worker are not pinned; they take `8800` and up, in enable order. Docker Compose publishes only `8787` (gateway) and `8794` (dashboard) to the host.

| Worker | `hoox dev` port | Endpoint URL | Purpose |
| :-------------------- | :-------------: | :---------------------- | :-------------------------------- |
| **`hoox`** | `8787` | `http://localhost:8787` | Public Gateway & Webhook Receiver |
| **`trade-worker`** | `8788` | `http://localhost:8788` | Trade Execution Engine |
| **`d1-worker`** | `8789` | `http://localhost:8789` | SQLite Database Operations |
| **`telegram-worker`** | `8790` | `http://localhost:8790` | Telegram Bot Alerts & Commands |
| **`web3-wallet-worker`** | `8792` | `http://localhost:8792` | On-Chain DeFi Execution |
| **`dashboard`** | `8794` | `http://localhost:8794` | Next.js Dashboard Cockpit |
| **`pyne-worker`** | `8799` | `http://localhost:8799` | Pine Script™ edge evaluate (PYNE) |
...
[STARTED] bun test runner... [PASSED]
...
- **[Testing Standards](/docs/enduser/guides/testing)** — Run unit and integration tests using Bun's native test runner.
- **[Terminal UI Operations](/docs/enduser/guides/tui)** — Launch the full-screen terminal cockpit (`./hoox-tui`) to monitor these local runs.
- **[Database Migrations](/docs/enduser/guides/database-ops)** — Set up, query, and migrate your local SQLite D1 database.
