...
DEX spots (`uniswap-ethereum`, `uniswap-arbitrum`, `jupiter-solana`) do not use CEX API keys. `hoox setup` writes `WALLET_EXECUTE_KEY_BINDING` and a Solana seed if missing. You still set `JUPITER_API_KEY` for Solana. See [DEX spot swaps](/docs/enduser/guides/dex-spot-swaps).
...
- **[5-Minute Quick Start Guide](/docs/enduser/getting-started/quick-start)** — Launch local worker runners and execute a simulated webhook trade signal.
- **[Deploying to Production](/docs/enduser/guides/deploy-workers)** — Deploy and bind all workers in the correct dependency sequence.
