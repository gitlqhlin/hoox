...
export interface WebhookSignal {
  ...
  exchange:
    ...
    | "uniswap-ethereum"
    | "uniswap-arbitrum"
    | "jupiter-solana"; // CEX or DEX venue
   ...
   * testnet, Bybit testnet). MEXC and DEX venues reject the request.
   * Prefer dedicated BINANCE_TESTNET_* / BYBIT_TESTNET_* secrets.
   */
  test?: boolean;
}
```
...
| `exchange`       | `string`  | **Yes**  | CEX: `binance`, `bybit`, `mexc`. DEX: `uniswap-ethereum`, `uniswap-arbitrum`, `jupiter-solana` |
| `action`         | `string`  | **Yes**  | One of: `LONG`, `SHORT`, `CLOSE_LONG`, `CLOSE_SHORT`                                  |
| `symbol`         | `string`  | **Yes**  | CEX: `BTCUSDT` (dashes/slashes stripped). DEX: **`BASE/QUOTE`** required (`ETH/USDC`) |
...
| `test`           | `boolean` |    No    | `true` → testnet for Binance/Bybit; MEXC and DEX venues return `TEST_TRADING_UNSUPPORTED` |
...
| Uniswap Ethereum / Arbitrum | No | Rejected (mainnet-only) |
| Jupiter Solana | No | Rejected (mainnet-only) |
...
Full operator guide: **[Test Trading](/docs/enduser/guides/test-trading)**. DEX spots: **[DEX spot swaps](/docs/enduser/guides/dex-spot-swaps)**.
...
- **[Autonomous AI Risk Management](/docs/enduser/concepts/ai-risk-manager)** — Learn how agent-worker tracks D1 entries to manage trailing stops and drawdowns.
- **[CLI Reference Manual](/docs/enduser/reference/cli-commands)** — Review commands to manage D1 tables, perform dry runs, and tail logs.
- **[Error Models & Status Codes](/docs/enduser/reference/api-endpoints#d-standard-platform-error-models)** — Understand all gateway error responses.
