...
External Webhook → hoox → [Queue] → trade-worker → D1 (via d1-worker)
                                      ├─ CEX HMAC REST (Binance / Bybit / MEXC)
                                      └─ WEB3_WALLET_SERVICE POST /swap (Uniswap V3 / Jupiter)
                        ↓
...
| --------------------- | ------------------ | -------------------- | ------------------------------------------------- |
| **D1 Database**       | `trade-data-db`    | `DB`                 | d1-worker only. Others use service binding `D1_SERVICE`. |
...
| **Vectorize**         | `my-rag-index`     | `VECTORIZE_INDEX`    | hoox, telegram-worker (768-d, `@cf/baai/bge-base-en-v1.5`) |
| **Analytics Engine**  | `hoox-analytics`   | `ANALYTICS_ENGINE`   | analytics-worker (`analytics_engine_datasets`)    |
...
| **Browser Rendering** | —                  | `BROWSER`            | report-worker (`quickAction("pdf")`, not the REST API) |
| **AI**                | —                  | `AI`                 | hoox, telegram-worker, agent-worker, dashboard    |
...
| `WALLET_EXECUTE_KEY_BINDING` | trade-worker, web3-wallet-worker, hoox | `hoox setup` (auto) | DEX P1 | Mesh key for wallet `/swap` (falls back to INTERNAL_KEY_BINDING) |
| `SOLANA_PRIVATE_KEY`      | web3-wallet-worker                  | `hoox setup` (auto if unset) | Optional | Solana seed (base58). Replace with a Phantom export to use an existing wallet |
| `JUPITER_API_KEY`         | web3-wallet-worker                  | `hoox secrets set` / init wizard | Solana DEX | Jupiter Swap V2 key from developers.jup.ag/portal |
| `MAILGUN_API_KEY`         | email-worker                        | `wrangler secret put` | Production   | HMAC key for `POST /webhook`           |
| `EMAIL_HOST`              | email-worker                        | `wrangler secret put` | Reserved     | Not read. IMAP polling is not implemented. |
| `EMAIL_USER`              | email-worker                        | `wrangler secret put` | Reserved     | Not read.                              |
| `EMAIL_PASS`              | email-worker                        | `wrangler secret put` | Reserved     | Not read.                              |
...
```jsonc
{
  ...
  "workers": {
    ...
    "email-worker": {
      ...
      "vars": {},
      "secrets": ["MAILGUN_API_KEY", "INTERNAL_KEY_BINDING"],
    },
    "analytics-worker": {
      "enabled": true,
...
| `email:scan_subject`                         | string  | —       | unused       | constant only; worker does not filter subjects |
...
| `email:use_imap`                             | boolean | —       | unused       | constant only; IMAP polling is not implemented |
