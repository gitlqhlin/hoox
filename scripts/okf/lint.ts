...
function checkGenerated(
  ...
  if (!isRecord(value)) {
    push(
      ...
      "E006",
      "error",
      "generated must be a mapping with by"
    );
    return;
  }
  checkActor(findings, path, "generated.by", value.by);
  if ("at" in value && value.at != null)
    checkTimestamp(findings, path, "generated.at", value.at);
}
...
function checkVerified(
  ...
  if (!Array.isArray(value) && !isRecord(value)) {
    push(
      ...
      "E009",
      "error",
      "verified must be an event mapping or a list of events"
    );
    return;
  }
  ...
  for (const event of events) {
    if (!isRecord(event)) {
      push(
        ...
        "E009",
        "error",
        "each verified entry must be a mapping with by and at"
      );
      continue;
    }
...
function checkSources(findings: Finding[], path: string, value: unknown): void {
  ...
  for (const entry of value) {
    ...
    if ("author" in entry && entry.author != null)
      checkActor(findings, path, "sources.author", entry.author);
    ...
    if (
      ...
      push(
        ...
        "E008",
        "error",
        "sources.usage_count must be a number"
      );
    }
  ...
  for (const key of ["executor", "attester"] as const) {
    ...
    if (key === "executor" && "receipt" in block && block.receipt != null) {
      ...
      if (
        ...
        receipt.some((item) => typeof item !== "string")
      ) {
        push(
          findings,
          path,
          "E015",
          "error",
          "executor.receipt must be a list of strings"
        );
      }
    }
  }
...
function checkConcept(
  ...
  if (
    ...
    if (fm.description.length > 280) {
      push(
        ...
        "W012",
        "warning",
        "description is longer than one sentence"
      );
    }
  }
  if (
  ...
  if ("tags" in fm && fm.tags != null) {
    if (
      ...
      fm.tags.some((tag) => typeof tag !== "string")
    ) {
      ...
      for (const tag of fm.tags) {
        if (typeof tag === "string" && !/^[a-z0-9-]+$/.test(tag)) {
          push(
            ...
            "W011",
            "warning",
            `tag "${tag}" should be lowercase words and hyphens`
          );
        }
      }
    }
  ...
  if ("generated" in fm && fm.generated != null)
    checkGenerated(findings, path, fm.generated);
  if ("verified" in fm && fm.verified != null)
    checkVerified(findings, path, fm.verified);
  if ("sources" in fm && fm.sources != null)
    checkSources(findings, path, fm.sources);
  if ("usage_window" in fm && fm.usage_window != null)
    checkUsageWindow(findings, path, fm.usage_window);
  if ("stale_after" in fm && fm.stale_after != null)
    checkTimestamp(findings, path, "stale_after", fm.stale_after);
  if (typeof fm.type === "string" && fm.type.trim() === "Attested Computation")
    checkAttested(findings, path, fm);
  if ("timestamp" in fm && !("generated" in fm)) {
    push(
      ...
      "W010",
      "warning",
      "timestamp is the v0.1 field; v0.2 uses generated.at"
    );
  }
  if (body.trim() === "")
    push(findings, path, "W007", "warning", "concept body is empty");
}
  ...
  if (root && yaml != null) {
    ...
    try {
      ...
      push(
        ...
        "E002",
        "error",
        "index frontmatter is not parseable YAML"
      );
      return;
    }
    if (!isRecord(parsed)) {
      push(
        ...
        "E004",
        "error",
        "root index frontmatter must be a mapping"
      );
      return;
    }
    ...
    if (keys.some((key) => key !== "okf_version")) {
      push(
        ...
        "E004",
        "error",
        "root index frontmatter may only contain okf_version"
      );
    }
    if ("okf_version" in parsed) {
      ...
      if (typeof version !== "string" && typeof version !== "number") {
        ...
        push(
          ...
          "W014",
          "warning",
          `okf_version ${String(version)} is not 0.2; checks still follow v0.2`
        );
      }
    }
  }
  ...
  for (const line of text.split("\n")) {
    ...
    if (line.startsWith("## ") && !match) {
      push(
        ...
        "E005",
        "error",
        `log heading "${line.slice(3)}" is not YYYY-MM-DD`
      );
      continue;
    }
...
export function resolveBundleLink(
  ...
  if (
    !bare ||
    bare.startsWith("http://") ||
    bare.startsWith("https://") ||
    bare.startsWith("mailto:")
  )
    return null;
  let rel = bare.startsWith("/")
    ? bare.slice(1)
  ...
  else if (!rel.endsWith(".md") && files.has(`${rel}/index.md`))
    rel = `${rel}/index.md`;
  return rel;
}
  ...
  for (const target of markdownLinkTargets(text)) {
    ...
    if (!reserved && !target.startsWith("/")) {
      push(
        ...
        "W006",
        "warning",
        `prefer a bundle-absolute link over ${target}`
      );
    }
  }
}
...
function reachable(files: Map<string, string>): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const target of markdownLinkTargets(text)) {
      ...
      if (resolved && names.has(resolved) && !seen.has(resolved))
        queue.push(resolved);
    }
  }
  return seen;
...
export function lintFiles(files: Map<string, string>): Finding[] {
  ...
  if (!files.has("log.md"))
    push(findings, "log.md", "W009", "warning", "bundle has no root log.md");
  ...
  for (const [path, text] of files) {
    ...
    if (yaml == null) {
      push(
        ...
        "E001",
        "error",
        "concept is missing YAML frontmatter"
      );
      continue;
    }
    let parsed: unknown;
  ...
  for (const path of files.keys()) {
    ...
    if (!reached.has(path))
      push(
        ...
        "W005",
        "warning",
        "concept is not reachable from the root index"
      );
  }
