...
The **`email-worker`** ingests trading signals from email sources, parses them, and forwards them to `trade-worker` for execution. It receives mail three ways: Mailgun HMAC on `POST /webhook`, the Cloudflare® Email Routing `email` handler, or internal JSON on `POST /email-signal` (mesh auth only — not Mailgun HMAC). No IMAP/SMTP polling — Cloudflare® Workers cannot open IMAP sockets. `EMAIL_HOST`, `EMAIL_USER`, and `EMAIL_PASS` are reserved and unread.
...
JSON bodies may include `"test": true` to request exchange testnet execution (Binance/Bybit). The flag is forwarded on the `TRADE_SERVICE` webhook call. **Plaintext extraction does not set `test`** (always live) — use structured JSON email bodies for sandbox trades. See [Test Trading](/docs/enduser/guides/test-trading).
...
- **[trade-worker Profile](/docs/devops/workers/trade-worker)** — How parsed signals become execution orders
- **[analytics-worker Profile](/docs/devops/workers/analytics-worker)** — Signal tracking and observability
- **[Architecture Overview](/docs/devops/architecture/overview)** — Full system architecture and data flow
