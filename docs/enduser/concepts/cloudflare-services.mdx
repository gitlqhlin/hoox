...
- **Concept**: Serverless Chrome at the Cloudflare® edge, bound to a worker as `BROWSER`.
- **Hoox Implementation**: `report-worker` calls `env.BROWSER.quickAction("pdf")` twice a day (08:00 and 18:00 UTC).
- **Why it Matters**:
  - Reads D1 P&L, renders an HTML report, writes the PDF to R2, and links it in the Telegram alert. Missing `BROWSER` falls back to a text summary.
...
- **[Idempotency Mechanics](/docs/enduser/concepts/idempotency)** — Dive into the V8 Durable Objects coordination engine.
- **[AI Risk Manager](/docs/enduser/concepts/ai-risk-manager)** — Understand how autonomous edge cron jobs evaluate drawdowns and manage trailing stops.
