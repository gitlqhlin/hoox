...
| Account · Vectorize | **Edit** | Gateway / Telegram RAG (`my-rag-index`, 768 dimensions) |
...
Exchange / Telegram / EVM wallet secrets stay empty until you supply them — that is expected. Mesh keys (`INTERNAL_KEY_BINDING`, `WALLET_EXECUTE_KEY_BINDING`, …) are generated by `hoox setup`. A Solana seed is generated if `SOLANA_PRIVATE_KEY` is unset. `JUPITER_API_KEY` is the remaining DEX secret you type.
