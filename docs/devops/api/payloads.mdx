...
export interface CexOrderPayload {
   ...
   * (Binance Futures testnet, Bybit testnet). MEXC and DEX venues
   * reject with TEST_TRADING_UNSUPPORTED.
...
| -------- | --------- | :------: | --------------------------------------------------------------------- |
| `test`   | `boolean` |    No    | `true` → testnet host for Binance/Bybit; rejected for MEXC and DEX    |
| `symbol` | `string`  | **Yes**  | CEX compact ticker or DEX `BASE/QUOTE` (`ETH/USDC`)                   |

DEX venues (`uniswap-ethereum`, `uniswap-arbitrum`, `jupiter-solana`) are ExactIn spots through `web3-wallet-worker`. See [DEX spot swaps](/docs/enduser/guides/dex-spot-swaps).
...
See [Test Trading](/docs/enduser/guides/test-trading) for operator setup.
...
- **[API Endpoint Directory](/docs/devops/api/endpoints)** — Analyze REST routes, headers, and endpoints mappings.
- **[Standard Response Schemas](/docs/devops/api/responses)** — Check success envelopes and error models.
