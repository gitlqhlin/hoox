...
- **Endpoint**: `/query`
- **Method**: `POST`
- **Notes**: **SELECT only**. Free-form writes are rejected — use named `/rpc/*` mutations.
- **JSON Payload**:
  ```json
  {
...
- **[System Storage Architecture](/docs/devops/architecture/storage)** — Review SQLite properties and R2 bucketing pipelines.
- **[Database Operations Manual](/docs/enduser/guides/database-ops)** — Learn commands to run query ledgers and restore backups.
