...
sequenceDiagram
    ...
    alt Rate limited
        ...
        alt Duplicate or in-flight
            ...
            Note over TW: Secrets + HMAC-SHA256 REST order (not WS order path)
            TW->>TW: Place order on exchange API
...
sequenceDiagram
    ...
    Cron->>RP: Fire trigger: 08:00 & 18:00 UTC
    RP->>RP: Query D1 P&L history & compile HTML5 template
    RP->>BR: BROWSER.quickAction("pdf", html)
    Note over BR: Browser Rendering binding returns a PDF buffer
