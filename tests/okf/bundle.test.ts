...
describe("OKF compile", () => {
  it("links an import to the dependency module and back", () => {
    ...
    expect(bundle.get("docs/architecture/overview.md")).toContain(
      "](/code/src/chart.md)"
    );
  });
  ...
  it("restamps and logs when a module changes", () => {
    ...
    expect(log.indexOf("## 2026-10-01")).toBeLessThan(
      log.indexOf("## 2026-09-24")
    );
  });
...
describe("OKF lint", () => {
  ...
  it("errors on a missing type, a bad log, and an attested computation without runtime", () => {
    ...
    expect(
      lintFiles(files).some(
        (finding) =>
          finding.path === "computations/revenue.md" && finding.rule === "E009"
      )
    ).toBe(false);
  });
  ...
  it("warns on a broken link and an orphan without failing conformance of the type", () => {
    ...
    expect(
      findings.some(
        (finding) => finding.rule === "W005" && finding.path === "c.md"
      )
    ).toBe(true);
...
describe("OKF apply", () => {
  it("writes the bundle and reports drift", () => {
    ...
    try {
      ...
      expect(
        lintBundle(dir).filter((finding) => finding.severity === "error")
      ).toEqual([]);
