...
sequenceDiagram
    autonumber
    participant Cron as agent-worker cron
    participant TW as trade-worker
    participant CEX as Binance / Bybit / MEXC
    participant D1 as d1-worker

    Cron->>TW: POST /api/positions/reconcile (X-Internal-Auth-Key)
    TW->>CEX: getPositions (signed REST)
    CEX-->>TW: Open interest snapshot
    TW->>D1: /api/dashboard/positions (OPEN rows)
    TW->>D1: /rpc/upsert-position for size changes / missing closes / new opens
    TW-->>Cron: totals { upserted, closed, unchanged }
```
...
When a deployed Pine strategy is scheduled for bar-close evaluation, **pyne-worker** runs on cron, loads newer OHLCV bars from R2, evaluates the script, and forwards actionable strategy events to **trade-worker** over the `TRADE_SERVICE` binding with mesh internal auth and an idempotency key.
...
- **[Storage Engineering Manual](/docs/devops/architecture/storage)** — Dive into Drizzle database schemas and SQLite properties.
