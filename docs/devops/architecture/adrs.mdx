...
**Status:** Accepted (evolved v0.13: two-phase + fail-closed)

**Context:** Webhook-based trade execution is inherently unreliable — network timeouts can cause TradingView® to retry the same signal, potentially resulting in duplicate trades. Gateways need **at-most-once acceptance** within a TTL without permanently blocking legitimate retries after soft failures.

**Decision:** Implement idempotency using Cloudflare Durable Objects with SQLite-backed state (`IdempotencyStore`). Keys prefer client `idempotencyKey` / `Idempotency-Key`; else a composite fingerprint with live/test mode and a **per-minute time bucket**. Flow is **two-phase**: `reserve` (pending) before queue/service work, `commit` only on true success, `release` on soft-fail or error. Missing DO binding is **fail-closed**. A separate `RateLimiterStore` DO provides atomic multi-isolate rate limits.

**Consequences:**

- **Positive:** Single-threaded DO isolation; in-flight retries blocked; failed trades can retry after release
- **Negative:** Extra DO RPCs per request; fail-closed can refuse trades if DO misconfigured
- **Mitigation:** Sharded object names; TTL alarms for GC; deploy checklist for migrations `v1`/`v2`
...
| `trade-worker`       | CEX order execution + DEX spots via wallet binding |
...
| `email-worker`       | Email signal parsing (Mailgun / Email Routing)  |
